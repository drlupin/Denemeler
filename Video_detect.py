  ...
  for box, color in six.iteritems(box_to_color_map):
    ...
    if keypoints is not None:
      ...
      def read_label_map(file):
          ...
          f.close()
          return dict([(b[b.index('id') + 3:b.index('name')].strip(),
                        b[b.index('name') + 5:b.index('}')].strip().replace("'", "")) for b in data_lst])

      category_index = read_label_map('sheep_label_map.pbtxt')  # put name of labelmap file here
      PATH_TO_SAVED_MODEL = "drive/MyDrive/saved_model"

      detect_fn = tf.saved_model.load(PATH_TO_SAVED_MODEL)
      cap = cv2.VideoCapture('11.mp4')  # put filenname of video here
      writer = cv2.VideoWriter("detection_output.avi", cv2.VideoWriter_fourcc('M', 'P', '4', 'V'), 30, (1280, 720))
      while cap.isOpened():
          ret, frame = cap.read()
          if ret:
